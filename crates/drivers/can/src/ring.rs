//! no_std 环形缓冲（D4 偏差）.
//!
//! 蓝图假设 `RingBuffer<CanFrame, 64>` 来自某外部库，但 no_std 环境下无该类型。
//! 本模块实现 const generics 环形缓冲 `RingBuffer<T, const N: usize>`，
//! 无外部依赖，使用固定大小数组 + 读/写指针。
//!
//! 遵循 Surgical Changes 原则：不跨 crate 共享 RS485 的 ring.rs，避免 crate 间耦合。
...
/// 支持单生产者-单消费者（SPSC）场景，用于 CAN 接收缓冲。
///
/// # 示例
/// ```
/// use eneros_can::RingBuffer;
