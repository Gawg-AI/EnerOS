...
> - **v0.23.0 电力协议原生支持**：GOOSE/SV AF_PACKET + 串口协议（规划中）
> - **v0.24.0 安全加固**：Secure Boot + seccomp + 审计 + 密钥管理（规划中）
> - **v0.25.0 高可用基础**：双节点心跳 + 状态同步 + 脑裂防护（规划中）
> - **v0.26.0 高可用切换**：热备接管 + 服务降级 + 多节点集群（规划中）
> - **v0.27.0 插件系统**：动态库 + 签名验证 + 沙箱 + 协议/Agent/分析插件（规划中）
> - **v0.28.0 开发者工具**：Rust SDK + 模拟器 + CLI + 文档体系（规划中）
> - **v0.29.0 数字孪生与高级 API**：数字孪生 + What-If + GraphQL + API 版本管理（规划中）
> - **v0.30.0 生态成熟**：认证 + 合规测试 + 端到端测试 + 混沌工程（规划中）
...
| v0.23.0 | 电力协议原生支持 | 规划中 | GOOSE/SV AF_PACKET + IEC 104/Modbus 串口 + 协议时间戳 + 冗余路径 | 规划中 |
| v0.24.0 | 安全加固 | 规划中 | Secure Boot + 内核加固 + seccomp + 审计系统 + 密钥管理 | 规划中 |
| v0.25.0 | 高可用基础 | 规划中 | 双节点心跳 + 状态同步 + 共享存储 + 脑裂防护 | 规划中 |
| v0.26.0 | 高可用切换 | 规划中 | 热备接管 + 服务降级 + 自动恢复 + 多节点集群 + 灾备演练 | 规划中 |
| v0.27.0 | 插件系统 | 规划中 | 动态库加载 + 签名验证 + seccomp 沙箱 + 协议/Agent/分析插件 | 规划中 |
| v0.28.0 | 开发者工具 | 规划中 | Rust SDK + 模拟器 + enerosctl 全功能 + 文档体系 | 规划中 |
| v0.29.0 | 数字孪生与高级 API | 规划中 | 数字孪生引擎 + What-If 推演 + 历史回放 + GraphQL + API 版本管理 | 规划中 |
| v0.30.0 | 生态成熟 | 规划中 | 认证体系 + 合规测试 + 端到端测试 + 混沌工程 + 协议一致性 + 性能基准 | 规划中 |
...
- 新增 `eneros-topology::connection_modes` 模块（7 种接线模式 + SAIFI/SAIDI + match_network）
- 14 个单元测试
...
- [x] `DeviceCommandExecutor` 持有 `DeviceManager` 引用，write 后 read_back 验证
...
- [x] `RuleBasedPlanner`：4 个规则模板（voltage_violation / overload / frequency_deviation / restore_supply）
...
- [x] `Lesson` 结构：scenario + failure_reason + improvement + importance
- [x] Lesson 自动存入 `AgentMemory`（procedural memory 类型）
- [x] `LearningPolicy`：控制学习频率（每 N 次执行学习一次）+ 最小重要性阈值
- [x] `generate_improvement_suggestion()` 按 goal_type 生成改进建议
...
- [x] 支持所有原有过滤器（memory_type / min_importance / tags / time_range）
...
**方案**：
- [x] `GET /api/timeseries/query` — 时序数据查询（element_id + parameter + time_range + aggregation）
...
- [ ] `POST /api/validation/check` — 系统级校验（v0.3.0 validation_rules）— 推迟到 v0.7.0
...
**方案**：
- [ ] 支持双点信息（M_DP_NA_1）、步位置信息（M_ST_NA_1）、BCR 总计
...
- [x] state_estimation、powerflow、analysis 迁移到稀疏接口
...
- [x] 安全字段热重载（log_level、enable_metrics、scada intervals、emergency 阈值、powerflow 参数）
- [x] `POST /api/config/reload` 手动重载端点
- [x] `GET /api/config` 运行时配置查看（敏感字段脱敏）
- **验收**：修改 eneros.toml 后 2 秒内 log_level 生效 ✅
...
- [x] `Drop` trait 防止任务泄漏
- [x] `DualScanOptions` 消除硬编码（timeout_ms、enable_quality_check、event_bus）
...
- [x] 3 个并发测试验证（不同设备 < 200ms、同设备 >= 200ms、无 device_id 兜底）
- **验收**：`cargo test -p eneros-gateway` 133 项通过 ✅

### 任务 3：时序配置接线 ✅

- [x] `compute_retention_capacity()` 函数从配置计算 max_retention（上限 1000 万点）
...
- **验收**：Observer Agent 进程无法 open(/dev/ttyS0, O_WRONLY)（Linux 环境验证，单元测试验证映射逻辑）
...
- [x] `SchedulingPolicy::Normal`（SCHED_OTHER）/ `Realtime`（SCHED_FIFO）
- [x] RT Agent：SCHED_FIFO + CPU 隔离 + mlockall
...
- [x] self-healing-agent 为 RT 进程（SCHED_FIFO）
- [x] **领域算法完整保留**
...
- [x] 保留 per-device 锁池、safety_checks、command_history
...
- SCHED_FIFO 优先级 80，CPU 隔离（isolcpus=2,3）
- mlockall 锁定内存
- rt/ipc.rs 真正无锁 SPSC（原子索引 + MaybeUninit）
- 硬件看门狗集成（/dev/watchdog，500ms 超时）
- 内核启动参数（isolcpus, nohz_full, rcu_nocbs）
- 实时性基准测试（P99 < 1ms）

### 验收标准
- RealtimeExecutor 线程运行在 SCHED_FIFO + 隔离 CPU
...
## v0.23.0 — 电力协议原生支持（规划中）

### 核心交付
- GOOSE 协议 AF_PACKET 原始套接字（Layer 2 直采）
- SV 协议 AF_PACKET（IEC 61850-9-2 LE 直采）
...
- 内核安全加固（HARDENED_USERCOPY + FORTIFY_SOURCE + 模块签名）
...
### 核心交付
- 双节点心跳（UDP 多播 100ms 间隔 + 300ms 故障检测）
- 状态同步（SCADA/Agent/命令历史/配置，延迟 < 100ms）
- 共享状态存储（DRBD 或自研复制）
- 脑裂防护（STONITH fencing）
- HA 配置管理
- enerosctl ha 子命令
...
## v0.29.0 — 数字孪生与高级 API（规划中）

### 核心交付
- 数字孪生引擎（实时电网镜像 + 拓扑镜像 + 持久化）
- What-If 场景推演（潮流/暂态/安全约束）
- 历史回放（速度控制 + 暂停/快进 + 对比分析）
- 虚拟传感器（计算公式 + 数据生成 + 校准）
- GraphQL API（schema + 查询/变更/订阅 + DataLoader）
...
### 核心交付
- 认证体系（IEC 62443 + 等保 2.0 三级）
- 安全合规测试套件（漏洞扫描 + 渗透测试 + 基线检查）
- 端到端集成测试（多协议/多Agent/HA/OTA/性能基准）
- 混沌工程测试（设备断连/网络分区/磁盘故障/过载/时钟漂移）
- 协议一致性测试套件（IEC 104/61850/Modbus/DNP3/OPC UA/GOOSE/SV）
- 性能基准测试（SCADA > 10000点/秒 + Agent < 100ms + HA < 3s）
- 测试覆盖率 > 80%
...
> v0.30.0 完成后，EnerOS 将具备与 Windows/Linux 齐平的 OS 功能，同时保持电力原生 Agent 系统特色。
> 后续版本将聚焦于：
> - 更多电力协议支持（IEC 60870-5-103/CDT/IEC 61850-90-5 R-GOOSE）
> - AI/ML 集成（异常检测/预测性维护/自适应优化）
> - 边缘计算（边缘 Agent + 联邦学习）
> - 云边协同（云端训练 + 边缘推理）
> - 国际化与多语言支持
...
3. **电力协议是 OS 能力**：GOOSE/SV 走 AF_PACKET，IEC 104 串口走 /dev/ttyS
